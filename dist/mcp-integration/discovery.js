...
export class MCPServerDiscovery {
    ...
    async discoverAvailableServers() {
        ...
        const discoveryPromises = discoveryMethods.map(method => method().catch(error => {
     ...
     */
    async discoverViaEnvironmentVariables() {
        const servers = [];
     ...
     */
    async discoverViaDefaultServers() {
        return [
            {
     ...
     */
    async analyzeServerCapabilities(serverName) {
        try {
     ...
     */
    async testServerAvailability(serverName) {
        try {
     ...
     */
    async getBestServerForTask(requirements) {
     ...
     */
    scoreServerForTask(server, requirements) {
        let score = 0;
    ...
    getServersByCategory(category) {
        return Array.from(this.servers.values()).filter(server => server.capabilities.categories.some(cat => cat.name === category));
    }
}
